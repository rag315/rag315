...
%% Problem 2: DFP Algorithm
% Global variables
global toll  
% Initial Values
x0 = [5, -5]; b = 100; a = 1; k = 0;
dx = 1; a_lo = 0; a_hi = 1;       % For Interval Halving
xmin = -5; xmax = 5;              % Ranges of x1
ymin = -35; ymax = 35;              % Ranges of x2
...
for i = 2:nX
    ...
    f = @(x) b*(x(2)-x(1))^2 + (a-x(1))^2;
    % Interval Halving
    [a_min, f_min, history] = inthalve(f, x(i,:), dx, a_lo, a_hi, toll, nmax);
    s = -B.*J(i,:);
    ggrad = x(i-1,:)+norm(s) - Gradient(x(i-1,:),b,a);
    g = Gradient(ggrad,b,a)';
...
title('Problem 2: DFP Rank 2 Algorithm')
xlabel('x1');ylabel('x2'); 
